= Testing for policy authors. :revdate: 2025-06-03 :page-revdate: {revdate} :description: An introduction to testing Kubewarden policies for policy authors. :doc-persona: ["kubewarden-policy-developer"] :doc-topic: ["testing-policies", "policy-authors"] :doc-type: ["tutorial"] :keywords: ["kubewarden", "policy testing", "policy author", "rust", "go", "assemblyscript", "development environment"] :sidebar_label: Policy authors :current-version: {page-origin-branch}
...
These Kubewarden policies are examples written in xref:tutorials/writing-policies/rust/01-intro-rust.adoc[Rust] and xref:tutorials/writing-policies/go/01-intro-go.adoc[Go]:
...
$ bats e2e.bats ✓ all is good

1 tests, 0 failures
---

xref:tutorials/writing-policies/go/05-e2e-tests.adoc[This] section of the documentation has more about writing end-to-end tests of your policies.